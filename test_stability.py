...
from config.excel1 import excel1
class RunTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        s1.oss_file_create()
        s1.version_create()
        s1.user_group_create()
        s1.version_factory_create()
        #s1.oss_file_create()
    #def __init__(self):
                ...
                if times>=2:
                    ...
                    while times >= 1:
                        ...
                        if header == "write":
                            ...
                            try:
                                res0 = self.run_method.run_main(method, url, request_data, herders)
                                # print(urls[0], url,type(json.loads(res)))
                                if "result" in res:
                                    # print(123456,type(res),res,321)
                                    #token = dict(res)["result"]["token"]
                                    token = dict(res)["result"]["accessToken"]
                                    # print("run",token)
                        ...
                        #time.sleep(5)
                        toc = time.clock()
                        tt = "响应时间: " + "%.2f" % (toc - tic) + " s"
                        tt1="响应时间: " + "%.2f" % (toc - tic)

                        if res["error code"] == 404:
                            print(123)
                            log1.error("次数:%s" % int(times)  +"-" + "%s" % tt1+ '\n')
                            #excel1.sheet_copy.write(times, 1, times)
                            #excel1.sheet_copy.write(times, 2, toc)
                            #excel1.sheet_copy.write(times, 3, tt)
                            with self.subTest(data=res):
                                self.assertEqual(res["code"], 200, "fail")
                        else:
                            log1.info("次数:%s" % int(times) + "-"+"%s"%res["error code"]+"-"+"%s" % tt + '\n')
                            #log1.info("响应body:%s" % res+ '\n')


                            #excel1.sheet_copy.write(times, 1, times)
                            #excel1.sheet_copy.write(times, 2, toc)
                            #excel1.sheet_copy.write(times, 3, tt)
                            # with self.subTest(data=times):
                            # self.assertEqual(res["code"], 200, "pass")

                        times -= 1
                    #print(t10)
                    #log1.info("响应时间超过5秒次数:%s" % t10+'\n')
                    #excel1.excel2.save(r'D:\python\python36-64\project\TWS-API\dataconfig\稳定性测试.xls')
                ...
                # 1次测试
                    ...
                    if expect != None:
                        ...
                        if type("123") == type("abc"):
                            ...
                            result = eval(res)
                            #print(result,11111111111111111111111)
                            #print(result["timestamp"],exp_dic["timestamp"])
                            # timestamp处理时间判断
                            ...
                            for keys in exp_dic.keys():
                                ...
                                # key对应value值为字典，以及字典判断
                                    ...
                                    for keys1 in exp_dic.get(keys).keys():
                                        ...
                                        # 第2层值为列表以及第3层列表判断exp_dic.get(keys).get(keys1)
                                        elif type(exp_dic.get(keys).get(keys1)) == type(
                                                ["abc", 123]) and "_list" not in keys1:
                                            for keys2 in exp_dic.get(keys).get(keys1)[0].keys():
                                                if "_length" in keys2:
                                                    ...
                                                    if result.get(keys).get(keys1)[0].get(key_1) != None:
                                                        if len(result.get(keys).get(keys1)[0].get(key_1)) == \
                                                            ...
                                                            print("%s.%s.%s:%s" % (keys, keys1, keys2,
                                                                                   result.get(keys).get(keys1)[0].get(
                                                                                       key_1)))
                                                    ...
                                                    if result.get(keys).get(keys1)[0].get(key_1) != None:
                                                        ...
                                                        result_exc.append(list1)
                                                # keys2值为字典判断
                                                elif type(exp_dic.get(keys).get(keys1)[0].get(keys2)) == type(
                                                        ["abc", 123]) and "_list" not in keys1:
                                                    for keys3 in exp_dic.get(keys).get(keys1)[0].get(keys2).keys():
                                                        if "_length" in keys3:
                                                            key_1 = keys3[:-7]
                                                            # print(key2,keys,result.get(keys))
                                                            if result.get(keys).get(keys1)[0].get(keys2).get(
                                                                    key_1) != None:
                                                                if len(result.get(keys).get(keys1)[0].get(keys2).get(
                                                                        key_1)) == \
                                                    ...
                                                    print(keys, keys1, keys2)
                                                    if result.get(keys).get(keys1)[0].get(keys2) == \
                                                            exp_dic.get(keys).get(keys1)[0].get(keys2):
                                                        pass
                                                        # print("%s pass" % keys2)
                                                    else:
                                                        print("%s.%s.%s:%s" % (
                                                        keys, keys1, keys2, result.get(keys).get(keys1)[0].get(keys2)))
                                                        list1 = {
                                                            "%s.%s.%s" % (keys, keys1, keys2):
                                                                result.get(keys).get(keys1)[0].get(keys2)}
                                                        result_exc.append(list1)
                                            ...
                                            if result.get(keys).get(key3) != None:
                                                if type(exp_dic.get(keys).get(keys1)) == type(
                                        ...
                                        # 第2层为字典以及对应第3层判断
                                            ...
                                            for keys2 in exp_dic.get(keys).get(keys1).keys():
                                                if "_length" in keys2:
                                                    ...
                                                    # print(keys, exp_dic.get(keys), key1)
                                                    if result.get(keys).get(keys1).get(key_1) != None:
                                                        if exp_dic.get(keys).get(keys1).get(keys2) in result.get(
                                                                keys).get(
                                                                keys1).get(key_1):
                                                            pass
                                                            # print("%s pass" % key_1)
                                                    ...
                                                    # print(key2,keys,result.get(keys))
                                                    if result.get(keys).get(keys1).get(key_1) != None:
                                                        if result.get(keys).get(keys1).get(key_1) in exp_dic.get(
                                                                keys).get(
                                                                keys1).get(keys2):
                                                            pass
                                                            # print("%s pass" % key_1)
                                                    ...
                                                    if result.get(keys).get(keys1).get(keys2) == exp_dic.get(keys).get(
                                                            keys1).get(keys2):
                                                        pass
                                                        # print("%s pass" % keys2)
                                                    else:
                                                        print("%s.%s.%s:%s" % (
                                                        keys, keys1, keys2, result.get(keys).get(keys1).get(keys2)))
                                                        list1 = {
                                                            "%s.%s.%s" % (keys, keys1, keys2):
                                ...
                                elif type(exp_dic.get(keys)) == type(
                                        ["abc", 123]) and "_list" not in keys and keys in result.keys():
                                    for keys1 in exp_dic.get(keys)[0].keys():
                                        if "_length" in keys1:
                                            key2 = keys1[:-7]
                                            # print(key2,keys,result.get(keys))
                                            if result.get(keys)[0].get(key2) != None:
                                                if len(result.get(keys)[0].get(key2)) == exp_dic.get(keys)[0].get(
                                                        keys1):
                                                    pass
                                                    # print("%s pass" % key2)
                                                else:
                                            ...
                                            for keys2 in exp_dic.get(keys)[0].get(keys1).keys():
                                                if "_length" in keys2:
                                                    ...
                                                    if result.get(keys)[0].get(keys1).get(key_1) != None:
                                                        if len(result.get(keys)[0].get(keys1).get(key_1)) == \
                                                                exp_dic.get(keys)[0].get(keys1).get(keys2):
                                                            pass
                                                            # print("%s pass" % key_1)
                                                        else:
                                                            print("%s.%s.%s:%s" % (
                                                    ...
                                                    # print(keys, exp_dic.get(keys), key1)
                                                    if result.get(keys)[0].get(keys1).get(key_1) != None:
                                                        if exp_dic.get(keys)[0].get(keys1).get(keys2) in \
                                                                result.get(keys)[
                                                    ...
                                                    # print(key2,keys,result.get(keys))
                                                    if result.get(keys)[0].get(keys1).get(key_1) != None:
                                                        if result.get(keys)[0].get(keys1).get(key_1) in \
                                                                exp_dic.get(keys)[
                                                    ...
                                                    if result.get(keys)[0].get(keys1).get(keys2) != None:
                                                        if result.get(keys)[0].get(keys1).get(keys2) == \
                                                                exp_dic.get(keys)[
                                                                    0].get(keys1).get(keys2):
                                                            pass
                                                            # print("%s pass" % keys2)
    ...
    def tearDown(self):
        ...
        s2.user_delete()
        s2.version_delete()
        s2.user_group_delete()
        s2.version_factory_delete()
